...
# Hay typo de propiedades con frecuencia muy baja. Cuento por tipo de propiedad y filtro las que tenga más de 50, solo se quedan 6 tipos de propiedades y la mayoría está en apartamento.
...
```{r}
ggplot(df_airbnb_work) + 
  geom_bar(aes(PRICE),fill = '#fd5c63',binwidth = 10) + 
  theme_minimal(base_size = 10) + xlab("Price") + ylab("Number") + 
  ggtitle("Distrubition of property price") 

# Vemos que la mayoría de propiedades tienen un precio inferior al rededor de 150€ Vemos que hay propiedades con precios muy superiores, que pueden llegar a un máximo de 875€, como vimos en el resumen del dataframe. Esto nos indica que hay outliers.
```

```{r}
ggplot(df_airbnb_work, aes(x = PROPERTY_TYPE, y = PRICE, fill = PROPERTY_TYPE)) + geom_boxplot(outlier.colour = "blue") + labs(x = "", y = "")
...
# Vemos que hay propiedades que están fuera de la caja, como si fueran outliers, pero puede sugerir que el precio no depende del tipo de propiedad, sino de otra variable.
```
```{r}

df_filter_property$BEDROOMS_FACT <- factor(df_filter_property$BEDROOMS)

ggplot(df_filter_property, aes(x = BEDROOMS_FACT, y = PRICE, fill = BEDROOMS_FACT)) + geom_boxplot(outlier.colour = "blue") + labs(x = "", y = "") +
  guides(fill = FALSE)  + 
  ggtitle("Price analysis by number of bedrooms")  

# Se puede ver que el precio es más alto cuando se incrementa el número de habitaciones. Vamos a analizar las propiedades con viviendas por encima de 6 habitaciones.
```
```{r}
prop_more_6_bedrooms <- df_filter_property |> filter(BEDROOMS > 5)

# Las propiedades con 6 habitaciones donde baja el precio se corresponde a viviendas en zona universitaria, donde el precio que se indica probablemente sea por persona para alojamiento compartido.
...
# Se puede ver que el precio es más alto cuando se incrementa el número de personas. El comportamiento es parecido al número de habitaciones.
```
...
```
```{r}
group_neigh_3Q <- group_neighbourhood |> filter(n_houses>78)
...
```{r}
group_neigh_3Q[order(-group_neigh_3Q$avg_price),]
```


```{r}
df_neigh_3Q <- group_neigh_3Q %>% left_join(df_filter_property, 
        by=c('NEIGHBOURHOOD_CLEANSED'='NEIGHBOURHOOD_CLEANSED'))
# Construyo df solo con los barrios filtrados arriba.
```

```{r}
est_price <- df_neigh_3Q[,c('PRICE', 'ZIPCODE','ACCOMMODATES','SECURITY_DEPOSIT','NUMBER_OF_REVIEWS','CANCELLATION_POLICY')]

library(GGally)
ggpairs(est_price[,c('PRICE', 'ZIPCODE','ACCOMMODATES','SECURITY_DEPOSIT','NUMBER_OF_REVIEWS','CANCELLATION_POLICY')],
...
```{r}
model_0 <- lm(data=est_price, formula = PRICE ~ ACCOMMODATES+SECURITY_DEPOSIT+NUMBER_OF_REVIEWS+ZIPCODE)
summary(model_0)
```
